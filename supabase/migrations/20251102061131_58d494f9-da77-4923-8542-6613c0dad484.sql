...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'workflow_task_type') THEN
...
DO $$ BEGIN
  ...
  DROP POLICY IF EXISTS "Only admins and managers can manage roles" ON public.roles;
  ...
  DROP POLICY IF EXISTS "Only admins and managers can assign roles" ON public.user_role_assignments;
  DROP POLICY IF EXISTS "Only admins and managers can update role assignments" ON public.user_role_assignments;
  DROP POLICY IF EXISTS "Offers are viewable by authenticated users" ON public.offers;
  DROP POLICY IF EXISTS "Marketing managers and admins can create offers" ON public.offers;
  DROP POLICY IF EXISTS "Marketing managers and admins can update offers" ON public.offers;
  DROP POLICY IF EXISTS "Users can view tasks assigned to them" ON public.workflow_tasks;
  DROP POLICY IF EXISTS "Managers and admins can create workflow tasks" ON public.workflow_tasks;
  DROP POLICY IF EXISTS "Assigned users can update their tasks" ON public.workflow_tasks;
  DROP POLICY IF EXISTS "Users can view their own audit logs" ON public.activity_audit;
  DROP POLICY IF EXISTS "System can insert audit logs" ON public.activity_audit;
EXCEPTION
...
CREATE POLICY "Only admins can insert brands" ON public.brands FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) = 'admin');
CREATE POLICY "Only admins can update brands" ON public.brands FOR UPDATE TO authenticated USING (public.get_user_role(auth.uid()) = 'admin');

-- RLS Policies for Profiles
CREATE POLICY "Users can view their own profile" ON public.profiles FOR SELECT TO authenticated USING (auth.uid() = id);
CREATE POLICY "Users can update their own profile" ON public.profiles FOR UPDATE TO authenticated USING (auth.uid() = id);
CREATE POLICY "Admins can view all profiles" ON public.profiles FOR SELECT TO authenticated USING (public.get_user_role(auth.uid()) = 'admin');
...
CREATE POLICY "Only admins and managers can manage roles" ON public.roles FOR ALL TO authenticated USING (public.get_user_role(auth.uid()) IN ('admin', 'manager'));
...
CREATE POLICY "Only admins can manage permissions" ON public.permissions FOR ALL TO authenticated USING (public.get_user_role(auth.uid()) = 'admin');
...
CREATE POLICY "Only admins can manage role permissions" ON public.role_permissions FOR ALL TO authenticated USING (public.get_user_role(auth.uid()) = 'admin');

-- RLS Policies for User Role Assignments
CREATE POLICY "Users can view their own role assignments" ON public.user_role_assignments FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.get_user_role(auth.uid()) IN ('admin', 'manager'));
CREATE POLICY "Only admins and managers can assign roles" ON public.user_role_assignments FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) IN ('admin', 'manager'));
CREATE POLICY "Only admins and managers can update role assignments" ON public.user_role_assignments FOR UPDATE TO authenticated USING (public.get_user_role(auth.uid()) IN ('admin', 'manager'));

-- RLS Policies for Offers
CREATE POLICY "Offers are viewable by authenticated users" ON public.offers FOR SELECT TO authenticated USING (true);
CREATE POLICY "Marketing managers and admins can create offers" ON public.offers FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) IN ('admin', 'marketing_manager', 'manager'));
CREATE POLICY "Marketing managers and admins can update offers" ON public.offers FOR UPDATE TO authenticated USING (public.get_user_role(auth.uid()) IN ('admin', 'marketing_manager', 'manager'));

-- RLS Policies for Workflow Tasks
CREATE POLICY "Users can view tasks assigned to them" ON public.workflow_tasks FOR SELECT TO authenticated USING (assigned_to_user_id = auth.uid() OR public.get_user_role(auth.uid()) IN ('admin', 'manager'));
CREATE POLICY "Managers and admins can create workflow tasks" ON public.workflow_tasks FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) IN ('admin', 'manager'));
CREATE POLICY "Assigned users can update their tasks" ON public.workflow_tasks FOR UPDATE TO authenticated USING (assigned_to_user_id = auth.uid() OR public.get_user_role(auth.uid()) IN ('admin', 'manager'));

-- RLS Policies for Activity Audit
CREATE POLICY "Users can view their own audit logs" ON public.activity_audit FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.get_user_role(auth.uid()) IN ('admin', 'manager'));
CREATE POLICY "System can insert audit logs" ON public.activity_audit FOR INSERT TO authenticated WITH CHECK (true);
...
-- Insert default brand (only if not exists)
INSERT INTO public.brands (name, code, status, description)
SELECT 'RetailPro', 'RETAIL_PRO', 'active', 'Default RetailPro Brand'
WHERE NOT EXISTS (SELECT 1 FROM public.brands WHERE code = 'RETAIL_PRO');

-- Insert default permissions (only if not exists)
INSERT INTO public.permissions (name, code, module_name, action_type, description)
SELECT * FROM (VALUES
...
) AS v(name, code, module_name, action_type, description)
...
DECLARE
  ...
  marketing_mgr_role_id UUID;
BEGIN
  SELECT id INTO default_brand_id FROM public.brands WHERE code = 'RETAIL_PRO' LIMIT 1;
  
  -- Insert roles if they don't exist
  INSERT INTO public.roles (name, code, description, brand_id, role_level)
  SELECT 'System Administrator', 'admin', 'Full system access', default_brand_id, 100
  WHERE NOT EXISTS (SELECT 1 FROM public.roles WHERE code = 'admin' AND brand_id = default_brand_id);
  
  INSERT INTO public.roles (name, code, description, brand_id, role_level)
  SELECT 'Store Manager', 'manager', 'Store-level operations management', default_brand_id, 80
  WHERE NOT EXISTS (SELECT 1 FROM public.roles WHERE code = 'manager' AND brand_id = default_brand_id);
  
  INSERT INTO public.roles (name, code, description, brand_id, role_level)
  SELECT 'Cashier', 'cashier', 'Transaction processing', default_brand_id, 20
  WHERE NOT EXISTS (SELECT 1 FROM public.roles WHERE code = 'cashier' AND brand_id = default_brand_id);
  
  INSERT INTO public.roles (name, code, description, brand_id, role_level)
  SELECT 'Stock Manager', 'stock_manager', 'Inventory management', default_brand_id, 50
  WHERE NOT EXISTS (SELECT 1 FROM public.roles WHERE code = 'stock_manager' AND brand_id = default_brand_id);
  
  INSERT INTO public.roles (name, code, description, brand_id, role_level)
  SELECT 'Marketing Manager', 'marketing_manager', 'Promotion and campaign management', default_brand_id, 60
  WHERE NOT EXISTS (SELECT 1 FROM public.roles WHERE code = 'marketing_manager' AND brand_id = default_brand_id);
  ...
  SELECT id INTO marketing_mgr_role_id FROM public.roles WHERE code = 'marketing_manager' AND brand_id = default_brand_id LIMIT 1;
  
  -- Assign all permissions to admin role
  INSERT INTO public.role_permissions (role_id, permission_id)
  SELECT admin_role_id, id FROM public.permissions
  WHERE NOT EXISTS (SELECT 1 FROM public.role_permissions WHERE role_id = admin_role_id);
  ...
  WHERE code IN ('view_dashboard', 'process_transaction', 'process_payment', 'apply_discount', 
    ...
    AND NOT EXISTS (SELECT 1 FROM public.role_permissions WHERE role_id = manager_role_id AND permission_id = permissions.id);
  ...
  WHERE code IN ('view_dashboard', 'process_transaction', 'process_payment', 'view_inventory', 'view_customers')
    AND NOT EXISTS (SELECT 1 FROM public.role_permissions WHERE role_id = cashier_role_id AND permission_id = permissions.id);
  ...
  WHERE code IN ('view_dashboard', 'view_inventory', 'adjust_stock', 'create_po', 'receive_goods')
    AND NOT EXISTS (SELECT 1 FROM public.role_permissions WHERE role_id = stock_mgr_role_id AND permission_id = permissions.id);
  
  INSERT INTO public.role_permissions (role_id, permission_id)
  SELECT marketing_mgr_role_id, id FROM public.permissions
  WHERE code IN ('view_dashboard', 'create_offers', 'approve_offers', 'view_reports')
    AND NOT EXISTS (SELECT 1 FROM public.role_permissions WHERE role_id = marketing_mgr_role_id AND permission_id = permissions.id);
END $$;